...
export default class ListEditor {
  ...
  toAppoint() {
    ...
    this.confirmPopup
      .getPopup('confirmDel')
      .addEventListener('click', (event) => this.onConfirmPopupClick(event));
  }
  ...
  onActionsClick(event) {
    ...
    if (event.target.dataset.name === 'name') {
      ...
      this.api.getTicketById(this.curId, (data) => {
        if (data) {
          this.description = event.target.querySelector(
  ...
  redrawTickets(data) {
    ...
    data.forEach((e) => {
      this.listTickets.insertAdjacentHTML(
        'beforeend',
        `<tr id="${e.id}">
      <td>
      ...
      <td data-name="date"><div class="td_content">${this.cleanDate(
    e.created,
  )}</div></td>
      <td>
